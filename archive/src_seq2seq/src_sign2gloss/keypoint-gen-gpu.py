...
CACHE_DIR = f"/mnt/d/dickmwong3/cache/{MODE}_3"
...
def mediapipe_extract(frames, override_random = False): # frames is a list of frames
    ...
    with PoseLandmarker.create_from_options(pose_base_option) as pose_landmarker:
        pose_results = [pose_landmarker.detect_for_video(Image(data=cv2.cvtColor(frames[i], cv2.COLOR_BGR2RGB), image_format=ImageFormat.SRGB), i) for i in range(len(frames))]
    with FaceLandmarker.create_from_options(face_base_option) as face_landmarker:
        face_results = [face_landmarker.detect_for_video(Image(data=cv2.cvtColor(frames[i], cv2.COLOR_BGR2RGB), image_format=ImageFormat.SRGB), i) for i in range(len(frames))]
    with HandLandmarker.create_from_options(hand_base_option) as hand_landmarker:
        hand_results = [hand_landmarker.detect_for_video(Image(data=cv2.cvtColor(frames[i], cv2.COLOR_BGR2RGB), image_format=ImageFormat.SRGB), i) for i in range(len(frames))]
    pose = np.array([res[:4] for result in pose_results for res in result.pose_landmarks]).flatten() if pose_results[0].pose_landmarks else np.zeros(33*4)
    face = np.array([res[:3] for result in face_results for res in result.face_landmarks]).flatten() if face_results[0].face_landmarks else np.zeros(478*3)
    hands = np.array([res[:3] for result in hand_results for res in result.hand_landmarks]).flatten() if hand_results[0].hand_landmarks else np.zeros(21*3*2)
    concat = np.concatenate([pose, face, hands])
    return concat[STATIC_KEYPOINTS_INDEX]
