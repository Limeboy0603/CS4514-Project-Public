...
def mediapipe_extract_single(results):
    ...
    class ?_LandmarkerResult:
        ?_landmarks: List[List[landmark_module.NormalizedLandmark]]
...
def apply_weighting_to_flattened(keypoint):
    ...
    # apply heavier weighting to hand keypoints
    keypoint[parts["left_hand"]] *= 1
    keypoint[parts["right_hand"]] *= 1
    # apply heavier weighting to pose keypoints
    keypoint[parts["pose"]] *= 1
    # scale up the entire keypoint by 100
    # keypoint *= 100
    return keypoint
